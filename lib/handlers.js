...
handlers._checkout.post = function(data, callback) {
	...
	var stripeToken = typeof(data.payload.stripeToken) == 'string' && data.payload.stripeToken.trim().length >= 1 ? data.payload.stripeToken.trim() : false;
	...
	if(orderId && stripeToken) {
		...
		_data.read('orders', orderId, function(err, orderData) {
			if(!err && orderData) {
				...
				var orderTotal = typeof(orderData.total) == 'number' && orderData.total > 1 ? orderData.total : false;
				var description = 'Pizza order from API';
				
				if(orderTotal) {
					// Charge the stripeToken and record the chargeId
					helpers.chargeToken(stripeToken, orderTotal, description, function(response) {
						var chargeId = typeof(response.chargeId) == 'string' && response.chargeId.trim().length > 0 ? response.chargeId.trim() : false;
						
						if(chargeId) {
							// Add the charge ID to the order
							orderData.chargeId = chargeId;
							
							// Store the new updates
							_data.update('orders', orderId, orderData, function(err) {
								if(!err) {
									// Email the receipt - Lookup the user, create message, send message
									...
									_data.read('users', orderData.userPhone, function(err, userData) {
										if(!err && userData) {
											...
											message += "We have received your order and charged your credit card $"+orderData.total+" for the following items:\n\n";
											orderData.longOrderData.forEach(function(item) {
												message += item.name+" ($"+item.price+")\n";
											});
											message += "\n";
											message += "Your order will be ready when you arrive.\n\n"
											message += "Thank you,\n\n"
											message += "-- Your Friendly Neighborhood Pizza Shop";
											
											// Send email
											helpers.sendEmail(userData.email, userData.firstName+' '+userData.lastName, 'Your pizza receipt', message, function(response) {
												if(!response.error) {
													callback(200, {'Success' : 'Order received'});
												} else {
													callback(500, {'Error' : 'Could not email receipt'})
												}
											});
